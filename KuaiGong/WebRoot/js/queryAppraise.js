...
function queryCount(){
	var id=$("#id").val();
	$.post("app/queryTheCount.action",{id:id},function(data){
		if(data!=null){
			$("#count").html(data);
		}
	},"json");
...
function queryAppraise(page){
	var id=$("#id").val();
	var page = page;
	var pageSize = 22;
	var b = new Base64();
	//��ȡ����
	$.post("app/QueryAll.action",{page:page,pageSize:pageSize,id:id},
		function(data) {
			if (data != null) {
				selectSum(Math.ceil(data/22));
			}
		}, "json");
	$.post("app/QueryUserPaGe.action",{page:page,pageSize:pageSize,id:id},
			function(data) {
				if (data != null) {
					$("#table_tbody").empty();
					$.each(data, function() {
						$("#table_tbody").append("<tr>"
								...
								+ "<td><a href='user/updateAppraise.jsp?id="+this.id+"&orderid="+this.orderid+"&username="+this.name+"&Eeusername="+this.Eeusername+ "&starlevel="+this.starlevel +"'>更新 </a>"+"</td>"
								+"<td><a href=javascript:void(0)  onclick =deleteByid("+this.id+") >删除</a>"+"</td>"
								+ "</tr>");
					});
				}
			}, "json");
