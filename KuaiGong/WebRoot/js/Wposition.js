...
function queryUserPage2(page){
	...
	var b = new Base64();
	//获取数量
	$.post("eelocation/queryUserPage.action",{page:page,pageSize:pageSize,query:query},
		function(data) {
			if (data != null) {
				selectSum(Math.ceil(data/22));
			}
		}, "json");
	$.post("eelocation/count.action",{query:query},function(data){
		if(data!=null){
			$("#count").html(data);
		}
	},"json");
	$.post("eelocation/queryUserInfo.action",{page:page,pageSize:pageSize,query:query},
			function(data) {
				if (data != null) {
					$("#table_tbody").empty();
					$.each(data, function() {
						$("#table_tbody").append("<tr>"
								+ "<td>" + this.id+ "</td>"
								+ "<td>" +b.decode(this.name)+ "</td>" + "<td>" +b.decode(this.username)+ "</td>" +
...
//没有加密 b.decode(this.username)
 ...
 function btuu(id,longitude,latitude){
	 ...
	 if(btu=="查看地理位置"){
			window.location.href="/KuaiGong/employee_userMap.jsp?longitude="+longitude+"&&latitude="+latitude+"&&eeid="+id;
		}
		if(btu=="删除"){
			var ss=confirm("确定要删除吗？");
			if(ss)
				{
				$.post("eelocation/delete.action",{id:id},function(data){
					if(data!=null){
						
						alert("删除成功！");
						window.location.href="Wposition.jsp";
					}
					else{
						alert("删除失败!");
					}
