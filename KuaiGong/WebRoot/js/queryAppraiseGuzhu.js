...
function queryUserPage(page){
	...
	var b = new Base64();
	//获取数量
	$.post("app/queryUserPage1.action",{page:page,pageSize:pageSize},
		function(data) {
			if (data != null) {
				selectSum(Math.ceil(data/8));
			}
		}, "json");
	$.post("app/queryUserInfo.action",{page:page,pageSize:pageSize},
			function(data) {
				if (data != null) {
					$("#table_tbody").empty();
					$.each(data, function() {
						$("#table_tbody").append("<tr>"
								...
								+ "<td>" +this.erid+ "</td>" 
								 + "<td>"
								+b.decode( this.starlevel) + "</td>" 
								+ "<td><a href='user/updateAppraise.jsp?id="+this.id+"&eeid="+this.erid+ "&starlevel="+this.starlevel +"'>修改 </a>"+"</td>"+"<td><a href=javascript:void(0)  onclick =deleteByid("+this.id+") >删除</a>"+"</td>"+ "</tr>");
							});
				}
			}, "json");
}
  ...
  function queryUserInfo(page){
	...
	$.post("app/queryUserInfo.action",{page:page,pageSize:pageSize},
		...
		if (data != null) {
			...
			$.each(data, function() {
				$("#table_tbody").append("<tr>"
						+ "<td>" + this.id+ "</td>"	+ "<td>" +this.erid+ "</td>" 
						 + "<td>"
						+b.decode( this.starlevel) + "</td>" 
						+ "<td><a href='user/updataUser.jsp?id="+this.id+"&eeid="+this.erid+"&starlevel="+this.starlevel +"'>修改 </a>"+"</td>"+"<td><a href=javascript:void(0)  onclick =deleteByid("+this.id+") >删除</a>"+"</td>"+ "</tr>");
					});
		}
	}, "json");
}
