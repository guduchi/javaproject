...
//	var b = new Base64();
	//获取数量
	$.post("euser/selectUserpage.action",{page:page,pageSize:pageSize,id:id},
		function(data) {
			if (data != null) {
				selectSum(Math.ceil(data/5));
			}
		}, "json");
	$.post("euser/selectLocationPaGe.action",{page:page,pageSize:pageSize,id:id},
			function(data) {
				if (data != null) {
					...
					$.each(data, function() {
						...
						$("#table_tbody_div").append(
								...
								+ "<div>" + b.decode(this.companyname)+ "</div>"
								+ "<div>" + b.decode(this.name)+ "</div>"
			        			 + "<div>" +b.decode(this.status)+ "</div>"
			        			 + "<div>" +this.credits+ "</div>"
								 +"<div>" + this.presentintegral+ "</div>"
								+"<div>" +"<button onclick='queryMAP("+this.longitude+","+this.latitude+")'>点击查看"+"</button>"+"</div>"
								 +"<div>"+b.decode(this.grade)+"</div>"
								 +"<div>" + test(this.starlevel)+ "</div>" 
								
								);
						$("#table_tbody_div2").append(
								 "<img src='http://www.dkuaigong.com/KuaiGongEmplyerApp/images/"+this.hand+ "'/>"
								);
				});
...
//"<td><a href=user/updataEmployerToAll.jsp>更新</a>"+

//没有加密 b.decode(this.username)
  function selectLocationPaGe(page){
	  var id=$("#id").val();
	var page = page;
	var pageSize=5;
 var b = new Base64();
...
//		alert(hand);
		window.location.href="/KuaiGong/user/map.jsp?longitude="+longitude+"&latitude="+latitude;
