...
function selectPay(){
	...
	$.post("OrderForm/paymentcancelOrder.action",{page:page,pageSize:pageSize,id:id,eename:eename,name:name,btu:btu,starttime:starttime,endtime:endtime},
			function(data) {
		if (data != null) {
			selectSum(Math.ceil(data/22),page);
		}														
	}, "json");
	$.post("OrderForm/cancelOrderCount.action",{id:id,name:name,eename:eename,starttime:starttime,endtime:endtime,btu:btu},function(data){
		if(data!=null){
			$("#count").html(data);
		}
	},"json");
	$.post("OrderForm/cancelOrderPage.action",{page:page,pageSize:pageSize,id:id,eename:eename,name:name,btu:btu,starttime:starttime,endtime:endtime},
			function(data) {
		if (data != null) {
			$("#table_tbody").empty();
			$.each(data,function() {
