...
function queryUserPage(page){
	...
	var endtime = $("#endtime").val();
	 ...
	 var GRusername = b.encode(GRusername);
	 $.post("OrderForm/selectOrderForm.action",{page:page,pageSize:pageSize,id:id,ee:eeid,ern:eename,erun:GRusername,status:status,starttime:starttime,endtime:endtime},
				function(data) {
					if (data != null) {
						selectSum(Math.ceil(data/22));
					}
				}, "json");
	 $.post("OrderForm/selectWorks.action",{id:id,ee:eeid,ern:eename,erun:GRusername,status:status,starttime:starttime,endtime:endtime},function(data){
			if(data!=null){
				$("#count").html(data);
			}
		},"json");
	 // 获取数
	$.post("OrderForm/selectOrderFormPage.action",{page:page,pageSize:pageSize,id:id,ee:eeid,ern:eename,erun:GRusername,status:status,starttime:starttime,endtime:endtime},
			function(data) {
		if (data != null) {
			$("#table_tbody").empty();
			$.each(data, function() {
...
function selectOrderFormPage(page){
	var b = new Base64();
	var page=page;
	var pageSize =22;
	var id = $("#id").val();
...
if(btu=="查看评价"){
	window.location.href="GROrderFormApp.jsp?id="+id;
}if(btu=="取消原因"){
